...
export default function ProfilePreview({
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      <div className="w-full max-w-[100%] mx-auto bg-[#0D0D0D] rounded-[22px] overflow-hidden shadow-md border border-[#222]">
        ...
        <div className="relative w-full bg-gradient-to-b from-[#1A1A1A] to-[#0D0D0D] p-6 flex flex-col items-center">
          ...
          {/* Bio */}
          <p className="text-base text-white/70 text-center mb-4">{bio || 'Trader'}</p>
          ...
          <div className="flex flex-wrap justify-center gap-2.5 mb-2">
            {(tags || ['#NFTTrader', '#DeFiWhale', '#Web3']).map((tag) => (
              <span 
                key={tag} 
                className={`px-3.5 py-1 rounded-full text-xs font-medium ${getTagColor(tag)}`}
              >
                {tag}
              </span>
            ))}
        ...
        {/* Stats Section - Added more vertical spacing */}
        <div className="w-full bg-[#0D0D0D] py-5 px-4">
        ...
        {/* CTA Section - More padding inside */}
        <div className="w-full bg-[#0A0A0A] p-4">
          <div className="bg-[#161616] rounded-xl p-5 text-center">
            ...
            <button className="bg-[#7048E8] hover:bg-[#6040D0] text-white py-2.5 px-7 rounded-full transition-colors text-sm font-medium">
              {links?.[0]?.cta.text || 'Check it out'}
            </button>
          </div>
        </div>
        
        {/* Footer with Tradr Logo - Slightly more padding */}
        <div className="w-full flex justify-center py-3.5 bg-[#080808]">
